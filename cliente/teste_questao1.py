...
class Ui_AtividadeProfessor(object):
    def setupUi(self, AtividadeProfessor, atividade_id=None, materia_id=None, turma_id=None):
        ...
        self.botao_add_questao.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
...
"background-color: rgb(0, 166, 202);")
        ...
        self.botao_add_questao.clicked.connect(self.func_botao_add_questao(1, 'enunciado', ['a', 'b', 'c', 'd', 'e']))
        self.retranslateUi(AtividadeProfessor)
        QtCore.QMetaObject.connectSlotsByName(AtividadeProfessor)

    def func_botao_add_questao(self, num, enunciado, alternativas, questao_id=None):
        def func_botao_add_questao():
            self.questoes[num] = AtividadeQuestao(self.scrollAreaWidgetContents, num, enunciado, alternativas, questao_id)
            self.questoes[num].input_enunciado.setPlaceholderText(self._translate("AtividadeProfessor", "Enunciado"))
            self.questoes[num].input_letra['A'].setPlaceholderText(self._translate("AtividadeProfessor", "A"))
            self.questoes[num].input_letra['B'].setPlaceholderText(self._translate("AtividadeProfessor", "B"))
            self.questoes[num].input_letra['C'].setPlaceholderText(self._translate("AtividadeProfessor", "C"))
            self.questoes[num].input_letra['D'].setPlaceholderText(self._translate("AtividadeProfessor", "D"))
            self.questoes[num].input_letra['E'].setPlaceholderText(self._translate("AtividadeProfessor", "E"))
            self.verticalLayout.addWidget(self.questoes[num])
        return func_botao_add_questao
    ...
    def retranslateUi(self, AtividadeProfessor):
        ...
        self.botao_add_questao.setText(self._translate("AtividadeProfessor", "+"))
